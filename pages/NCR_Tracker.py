...
sub1.plotly_chart(style_chart(px.pie(status_counts, values="Count", names="Status", title="NCR Open vs Closed")), width="stretch")

aging = ncr.groupby("Project")["Aging"].mean().reset_index()
sub2.plotly_chart(style_chart(px.bar(aging, x="Project", y="Aging", title="NCR Aging Analysis")), width="stretch")
...
if "Date Raised" in trend.columns:
    ...
    st.plotly_chart(style_chart(px.line(trend, x="Month", y="Count", color="Status", title="NCR Trend")), width="stretch")
...
if not open_by_project.empty:
    st.plotly_chart(style_chart(px.bar(open_by_project, x="Project", y="Open NCRs", title="Open NCRs by Project")), width="stretch")
...
st.plotly_chart(style_chart(px.bar(closeout_percentage, x="Project", y="Closeout %", title="NCR Closeout Percentage")), width="stretch")
