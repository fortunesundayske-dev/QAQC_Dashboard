...
def render_user_sidebar():
    """Render the authenticated user's compact sidebar account panel.

    This function is intentionally safe to call from legacy pages. The
    single-page app may choose to render its own sidebar instead; in that
    case calling this function simply renders the account block and logout
    control without changing navigation.
    """
    init_auth()
    ...
    user = current_user() or auth_state
    name = str(user.get("name") or user.get("username") or "User").strip()
    username = str(user.get("username") or auth_state.get("username") or "").strip()
    role = str(user.get("role") or auth_state.get("role") or "user").strip().replace("_", " ").title()
    email = str(user.get("email") or auth_state.get("email") or "").strip()
    photo = (
        user.get("profile_photo_asset")
        or user.get("profile_photo")
        or auth_state.get("profile_photo_asset")
        or auth_state.get("profile_photo")
    )

    with st.sidebar:
        if photo:
            if str(photo).startswith(("http://", "https://", "data:image/")):
        ...
        if st.button("Sign out", key="auth_render_user_sidebar_signout", width="stretch"):
            sign_out()
            st.rerun()
